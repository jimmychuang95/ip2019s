...
function analyze() {
    ...
    do {
      ...
      path = image.path(subs).fill("black").stroke({color:'gray',width:5}).draggable();
      ...
      for(var i = 0;i < (newPath.length - 1);i++){
        ...
        if(i==0){
          var circle = image.circle(20).fill('blue').stroke('black').move(newPath[i][1]-10,newPath[i][2]-10).draggable();
        }
        else {
          var circle = image.circle(10).fill('LightBlue').stroke('blue').move(newPath[i][1]-5,newPath[i][2]-5).draggable();
          var circle = image.circle(10).fill('LightBlue').stroke('blue').move(newPath[i][3]-5,newPath[i][4]-5).draggable();
          var circle = image.circle(10).fill('LightBlue').stroke('blue').move(newPath[i][5]-5,newPath[i][6]-5).draggable();
        }
      }
